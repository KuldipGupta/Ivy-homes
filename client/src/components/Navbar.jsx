...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: 'Saved', path: '/saved', icon: Heart, badge: favourites.length }
  ];
        ...
        <div className="md:hidden border-t border-slate-200 bg-white px-4 pt-3 pb-5 space-y-2">
          {isAuthenticated ? (
            ...
            <Link
              to="/login"
              onClick={() => setMobileMenuOpen(false)}
              className="block w-full text-center px-4 py-2.5 rounded-lg font-semibold text-white bg-emerald-600 hover:bg-emerald-700 shadow-sm"
            >
              Sign In
            </Link>
          )}
